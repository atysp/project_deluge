"""Task2 module for predicting median house prices based on postcode data."""
...
if __name__ == "__main__":
    ...
    train.drop(8713, inplace=True)
    X_train, X_test, y_train, y_test = train_test_split(train.drop(columns=['medianPrice']), train['medianPrice'], test_size=0.2, random_state=42)
    df = pd.concat([X_train, y_train], axis=1)
    print(df)
    model_instance = Task2(df)
